...
void  debug_print_Nnie_segInfo(SVP_NNIE_MODEL_S *pmodel)
{
#ifdef __DEBUG__
	for(int  i = 0;i< pmodel->u32NetSegNum;i++)
	{
	   printf("seg  [%d]  model  info            \r\n",i);
	   printf("seg  [%d]  net  type:%d           \r\n",i,pmodel->astSeg[i].enNetType);
	   printf("seg  [%d]  net  u16SrcNum     :%d \r\n",i,pmodel->astSeg[i].u16SrcNum);
	   printf("seg  [%d]  net  u16DstNum     :%d \r\n",i,pmodel->astSeg[i].u16DstNum);
	   printf("seg  [%d]  net  u16RoiPoolNum :%d \r\n",i,pmodel->astSeg[i].u16RoiPoolNum);

	  
	   for(int  j =0 ;j  < pmodel->astSeg[i].u16SrcNum;j++ )
	   {
	        printf("astSrcNode %d  type   %d           \r\n",j,pmodel->astSeg[i].astSrcNode[j].enType);
	        printf("astSrcNode %d  name %s           \r\n",j,pmodel->astSeg[i].astSrcNode[j].szName);
	        printf("astSrcNode %d  Width     %d \r\n",j,pmodel->astSeg[i].astSrcNode[j].unShape.stWhc.u32Width);
	        printf("astSrcNode %d  u32Height %d \r\n",j,pmodel->astSeg[i].astSrcNode[j].unShape.stWhc.u32Height);
	        printf("astSrcNode %d  u32Chn    %d \r\n",j,pmodel->astSeg[i].astSrcNode[j].unShape.stWhc.u32Chn);
	        printf("astSrcNode %d  dim       %d \r\n",j,pmodel->astSeg[i].astSrcNode[j].unShape.u32Dim);

	   }

	   for(int  j =0 ;j  < pmodel->astSeg[i].u16DstNum;j++ )
	   {
	        printf("astDstNode %d  type   %d          \r\n",j,pmodel->astSeg[i].astDstNode[j].enType);
	        printf("astDstNode %d  name %s           \r\n",j,pmodel->astSeg[i].astDstNode[j].szName);
	        printf("astDstNode %d  Width     %d \r\n",j,pmodel->astSeg[i].astDstNode[j].unShape.stWhc.u32Width);
	        printf("astDstNode %d  u32Height %d \r\n",j,pmodel->astSeg[i].astDstNode[j].unShape.stWhc.u32Height);
	        printf("astDstNode %d  u32Chn    %d \r\n",j,pmodel->astSeg[i].astDstNode[j].unShape.stWhc.u32Chn);
	        printf("astDstNode %d  dim       %d \r\n",j,pmodel->astSeg[i].astDstNode[j].unShape.u32Dim);
...
int _load_model(const char *model_path, SAMPLE_SVP_NNIE_MODEL_S *s_stModel)
{
    ...
    debug_print_Nnie_segInfo(&s_stModel->stModel);
    return 0;
}
...
void NNIE_Forward_From_Data(const unsigned char *data, SAMPLE_SVP_NNIE_MODEL_S *s_stModel,
                            SAMPLE_SVP_NNIE_PARAM_S *s_stNnieParam, Tensor output_tensors[])
{
    HI_S32 s32Ret = HI_SUCCESS;
...
void NNIE_Forward_From_Frame(IVE_IMAGE_S *pstframe, SAMPLE_SVP_NNIE_MODEL_S *s_stModel,
                            SAMPLE_SVP_NNIE_PARAM_S *s_stNnieParam, Tensor output_tensors[])
{
    HI_S32 s32Ret = HI_SUCCESS;
...
*/
    ...
    s_stNnieParam->astSegData[stInputDataIdx.u32SegIdx].astSrc[stInputDataIdx.u32NodeIdx].u64VirAddr = pstframe->au64VirAddr[0];
    s_stNnieParam->astSegData[stInputDataIdx.u32SegIdx].astSrc[stInputDataIdx.u32NodeIdx].u64PhyAddr = pstframe->au64PhyAddr[0];
    s_stNnieParam->astSegData[stInputDataIdx.u32SegIdx].astSrc[stInputDataIdx.u32NodeIdx].u32Stride  = pstframe->au32Stride[0];
